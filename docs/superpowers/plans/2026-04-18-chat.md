...
import { getSession, insertMessage, insertMember, insertSession, listActiveMembers, listMembers, listMessages, listSessions, markMemberLeft, nextSeq, patchMember as patchMemberRow, deleteSession as deleteSessionRow, setSessionArchived, getBudgetState, resetBudgetState } from "./repository"
import type { ChatBackend } from "../../agent/executor"
import type { GroupDeps } from "./group"
import type { AddMemberInput, AnswerAskInput, ChatBridge, ChatEvents, ChatMember, ChatMessage, ChatSession, CreateSessionInput, LoopEndedReason, LoopPausedEvent, MemberPatch } from "./types"
...
export class ChatService extends IpcController<ChatEvents> implements IpcLifecycle {
  ...
  @Handle("message:send") async sendUserMessage(sessionId: string, content: string): Promise<void> {
    ...
    insertMessage({ id: userMsg.id, sessionId, seq: userMsg.seq, openclawSessionKey: null, openclawMessageId: null, senderType: "user", senderId: null, role: "user", content: userMsg.content, mentions: userMsg.mentions, turnRunId: null, tags: [], createdAtRemote: null })
  ...
  private buildBackend(): ChatBackend {
    ...
    const backend: ChatBackend = {
      ...
      onEvent: (runId, listener) => {
        ...
        const off = getGateway().onEvent?.((frame: { type?: string; payload?: { runId?: string; seq?: number; state?: string; message?: { content?: { type: string; text?: string }[] }; stopReason?: string; usage?: Record<string, number>; errorMessage?: string; errorKind?: string } }) => {
          const p = frame.payload
          if (!p || p.runId !== runId) return
          const textJoin = (p.message?.content ?? []).filter((b) => b.type === "text").map((b) => b.text ?? "").join("")
  ...
  private async subscribePluginChatEvents(): Promise<() => void> {
    ...
    return onPluginCustom?.((ev: { channel: string; data: unknown }) => {
...
function parseMentionsFromText(content: string, members: ChatMember[]): ReturnType<typeof listMembers>[number]["replyMode"] extends never ? never : Array<{ agentId: string; source: "plain" }> {
  // 简化：匹配 "@<agent_id>" 字符串
