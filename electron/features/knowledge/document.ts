...
/** 弹出文件选择对话框 + 上传到指定知识库。 */
export async function uploadDocuments(_kbId: string, _onProgress: (event: DocProgressEvent) => void): Promise<KnowledgeDocRow[]> {
 ...
 */
export async function retryDocument(_docId: string, _onProgress: (event: DocProgressEvent) => void): Promise<void> {
  throw new Error("NOT_IMPLEMENTED")
}
