...
import type { InvokeArgs, InvokeResult, OpenClawStatus } from "./types"
...
import { checkCompatibility as computeCompatibility } from "./compatibility"
import { pushStatus } from "../status/service"
import { scanner, emitPluginEvent, emitPluginMonitor, setBridgeServer } from "../container"

const log = scope("openclaw:plugin")
...
export class PluginService extends IpcController implements IpcLifecycle {
  ...
  async onReady(): Promise<void> {
    ...
    server.onEvent((event) => {
      ...
      if (monitor) emitPluginMonitor(monitor)
      else emitPluginEvent(event)
    })
