 ...
 * 启动一个多阶段任务。
 * @param params 任务参数
 * @param deps 依赖注入：gateway 客户端 + 插件调用
 */
export async function startTask(params: StartTaskParams, deps: { client: GatewayClient; invoke: InvokePluginFn }): Promise<TaskState> {
  const taskId = `task-${++taskCounter}-${Date.now().toString(36)}`

  // 1. 创建 session
  const sessionKey = `agent:${params.agentId}:kaiwu:${taskId}`
  const result = (await deps.client.sessionCreate({ key: sessionKey, agentId: params.agentId, label: params.label })) as { key?: string }
...
/** 逐阶段执行。 */
async function runStages(state: TaskState, deps: { client: GatewayClient; invoke: InvokePluginFn }): Promise<void> {
  for (const stage of state.stages) {
    ...
    // 发消息
    await deps.client.chatSend({ sessionKey: state.sessionKey, message: stage.message })

    // 等 agent 完成（监听 final/error/aborted）
    await waitForCompletion(state.sessionKey, deps.client)
...
function waitForCompletion(sessionKey: string, client: GatewayClient): Promise<void> {
  return new Promise((resolve, reject) => {
    const unsub = client.subscribeChatEvent(sessionKey, (event) => {
 ...
 */
export async function abortTask(taskId: string, client: GatewayClient): Promise<void> {
  const state = tasks.get(taskId)
  if (!state || state.status !== "running") return
  state.status = "aborted"
  notifyChange(state)
  await client.chatAbort({ sessionKey: state.sessionKey }).catch(() => {})
}
