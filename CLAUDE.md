...
export class ChromeService extends IpcController<ChromeEvents> implements IpcLifecycle {
  ...
  @Handle("window:minimize") minimize() {
...
- `features/openclaw/service.ts` —— IPC 门面，字段组合 `gateway + host` 两个协作对象 + 内联 `emitEvent` 回调
  ...
  - `plugin/lifecycle.ts` —— 纯函数集:installBridge/uninstallBridge/detectStatus/restartGateway/checkCompatibility，由 service @Handle 直调；`plugin/invoke.ts` 放 HTTP invoke
- openclaw 内部按功能域分子目录（2 级扁平，单词文件名）：
  - `gateway/` —— 连接 + 协议 + 探测 + 域本地 types（socket/caller/emitter/manager/client/detection/deployment/port/lock/config/auth/handshake/contract/types）
  - `plugin/` —— 插件桥接 + 域本地 types（host/lifecycle/invoke/sync/transport/handshake/compat/dispatcher/security/types）
  - `agent/` `chat/` `session/` `model/` —— RPC 方法 + 各自 `contract.ts`（业务 RPC 契约），methods 函数走动词前置命名（`list` / `create` / `send` / `abort` 等），参数收 `GatewayClient` 调 `gateway.call<T>()`
