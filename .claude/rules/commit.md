...
风格：Conventional Commits + 中文描述。每条提交既能机器解析（type）又能让人看懂（中文正文）。
...
```
✅ feat: 失败自动重试避免单次网络抖动导致升级中断
✅ fix: macOS 关闭按钮触发 hide 而不是 quit 修复 dock 行为
✅ refactor: 移除 feature barrel 防止主进程代码污染 preload bundle
...
| 加新 feature 顺带升级依赖     | 拆成 `chore` + `feat`                                             |
...
```
fix: [hotfix] 回滚 1.2.3 的签名校验，旧版客户端无法升级
```
